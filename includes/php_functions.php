<?php 
    ...
    function set_message($msg, $level='danger') {
        if(($level != 'primary') && ($level != 'success') && ($level != 'info') && ($level != 'danger')) {
            $level = 'danger';
        }
        if(empty($msg)) {
    ...
    function send_email($to, $subject, $body, $from, $reply) {
        ...
        if ($_SERVER['SERVER_NAME'] != "localhost") {
            ...
            set_message("Se ha enviado un correo a {$to}. Revise su bandeja de entrada y correo no deseado.");
            redirect('index.php');
        } else {
    ...
    function update_login_date($pdo, $user) {
        try {
            $sql="UPDATE users SET last_login=current_date WHERE username =:username";
            $stmnt = $pdo->prepare($sql);
            $stmnt->execute([':username'=>$user]);
        } catch (PDOException $e) {
            return $e->getMessage();
