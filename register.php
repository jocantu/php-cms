...
<?php 
    if($_SERVER['REQUEST_METHOD']=="POST") {
        ...
        if(strlen($pword)<6) {
            $error[] = "La contrasena debe te tener por lo menos 6 caracteres";
        }
        if($pword != $pword_conf) {
        ...
        if (count_field_val($pdo, "users", "username", $uname)!= 0) {
            $error[] = "El usuario '{$uname}' ya existe";
        }

        if (count_field_val($pdo, "users", "email", $email)!= 0) {
            $error[] = "El correo '{$email}' ya esta registrado a una cuenta";
        }

        if (!isset($error)) {
            $vcode = generate_token();
            try {
                $sql = "INSERT INTO users (firstname, lastname, username, email, password, comments, validationcode, active, joined, last_login) VALUES (:firstname,:lastname,:username,:email,:password,:comments,:vcode,'false', current_date, current_date)";
                ...
                $body = "Para activar tu cuenta accede al siguiente enlace.\r\n
                http://{$_SERVER['SERVER_NAME']}/{$root_directory}/activate.php?user={$uname}&code={$vcode}";
                send_email($email, "Activacion de cuenta", $body, $from_email, $reply_email);
                // set_message("Usuario registrado exitosamente!","success");
                // redirect("index.php");
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <form id="register-form" method="post" role="form" >
                                        ...
                                        <div class="form-group">
                                            <input type="email" name="email_confirm" id="email_confirm" tabindex="4" class="form-control" placeholder="Confirm Email Address" value="<?php echo $email_conf;?>" required >
                                        </div>
                                        <div class="form-group">
                                            <input type="password" name="password" id="password" tabindex="5" class="form-control" placeholder="Password" value="<?php echo $pword_conf;?>" required>
                                        </div>
                                        <div class="form-group">
